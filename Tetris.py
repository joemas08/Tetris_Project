...
def create_grid(locked_positions={}):
    grid = [[(0, 0, 0) for _ in range(10)] for _ in range(20)]  # creating 2D sublist to fill the play grid
...
def draw_window(surface, grid, score=0, high_score = 0):
    ...
    surface.blit(label,
                 (top_left_x + play_width / 2 - (label.get_width() / 2), 30))  # x and y coordinate of Tetris label
    # current score
    font = pygame.font.SysFont('comicsans', 30)
    ...
    surface.blit(label, (sx + 20, sy + 160))  # x and y coordinate of score
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    if not (valid_space(current_piece, grid)):
                        current_piece.x += 1  # might need current_piece.x
